...
export default function PreferencesApp() {
  ...
  const { theme, pattern, dockSettings, setTheme, setPattern, updateDockSettings } = useOS()
        ...
        <div className="flex-1 overflow-auto space-y-4 pr-1">
          <section className="space-y-2">
            <div className="flex justify-between items-center border-b border-[var(--os-border)] pb-1">
              ...
              <span className="text-[10px] opacity-60">Pilih palet tampilan CRT</span>
            </div>
          ...
          <section className="space-y-2">
            <div className="flex justify-between items-center border-b border-[var(--os-border)] pb-1">
              <span className="font-bold">Pola Latar Desktop (Background Pattern)</span>
              <span className="text-[10px] opacity-60">Tekstur 1-bit khas Payaman OS</span>
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-3 items-center">
              ...
              <div className="flex-1 text-[11px] space-y-1 opacity-80">
                ...
                <p className="text-[10px] text-neutral-500 pt-1">
                  Perubahan langsung diterapkan pada seluruh jendela dan disimpan ke memori disk lokal.
                </p>
              </div>
            </div>
          </Panel>
        ...
        <div className="flex-1 overflow-auto space-y-4 pr-1">
          <section className="space-y-2">
            <div className="flex justify-between items-center border-b border-[var(--os-border)] pb-1">
              ...
              <span className="text-[10px] opacity-60">Pilih skala ukuran ikon pada dock</span>
            </div>
          ...
          <section className="space-y-2">
            <div className="flex justify-between items-center border-b border-[var(--os-border)] pb-1">
              ...
              <span className="text-[10px] opacity-60">Penempatan dock pada desktop</span>
            </div>
            <div className="grid grid-cols-3 gap-2">
              {DOCK_POSITIONS.map((pos) => {
                const isCurrent = (dockSettings?.position || 'bottom') === pos.id
                return (
                  <button
                    key={pos.id}
                    type="button"
                  ...
                  >
                    ...
                    <span className="text-[10px] opacity-70 text-center">{pos.description}</span>
                  </button>
                )
              })}
            </div>
          ...
          <Panel title="Pratinjau Dock (Dock Preview)">
            <div className="flex flex-col sm:flex-row gap-3 items-center">
              <div className="w-56 h-36 border-2 border-[var(--os-border)] bg-[var(--os-desktop-bg)] relative flex items-center justify-center overflow-hidden">
                ...
                >
                  <div className="w-3 h-3 border border-current flex items-center justify-center text-[6px]">1</div>
                  <div className="w-3 h-3 border border-current flex items-center justify-center text-[6px]">2</div>
                  <div className="w-3 h-3 border border-current flex items-center justify-center text-[6px]">3</div>
                  <div className="w-px bg-current opacity-30 mx-0.5" />
                  <div className="w-3 h-3 border border-current flex items-center justify-center text-[6px]">🗑</div>
                </div>
              </div>
              <div className="flex-1 text-[11px] space-y-1 opacity-80">
                <p>
                  <strong>Ukuran:</strong>{' '}
                  {DOCK_SIZES.find((s) => s.id === (dockSettings?.size || 'medium'))?.name} (
                  {DOCK_SIZES.find((s) => s.id === (dockSettings?.size || 'medium'))?.label})
                </p>
                <p>
                  <strong>Posisi:</strong>{' '}
                  {DOCK_POSITIONS.find((p) => p.id === (dockSettings?.position || 'bottom'))?.name}
                </p>
                <p>
          ...
          <Panel title="Informasi Lingkungan Payaman OS">
            <div className="space-y-2 text-[11px]">
              ...
              <div className="flex justify-between border-b border-[var(--os-border)] pb-1">
                ...
                <span className="font-bold">Modern Web Desktop Environment</span>
              </div>
              <div className="flex justify-between border-b border-[var(--os-border)] pb-1">
